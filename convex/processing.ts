...
async function fetchYouTubeTranscript(
  ...
  try {
    ...
    const transcriptResp = await fetchWithTimeout(`${captionUrl}&fmt=json3`, {
      timeout: 15000,
    });
...
async function tryInnerTubeClient(
  ...
  if (cookieHeader) {
    ...
    const isWebClient =
      clientConfig.clientName.startsWith("WEB") ||
      clientConfig.clientName.startsWith("TV");
    if (isWebClient) {
      ...
      if (sapisid) {
        ...
        console.log(
          `[YouTube] Added SAPISIDHASH auth for ${clientConfig.name}`,
        );
      }
    }
  }
...
const INNERTUBE_CLIENTS = [
  ...
  },
  {
    ...
    userAgent:
      "com.google.ios.youtube/20.03.02 (iPhone16,2; U; CPU iOS 18_3_2 like Mac OS X;)",
    clientNum: "5",
    extraContext: {
      deviceMake: "Apple",
...
async function getYouTubeStreamUrls(
  ...
  if (cookieHeader) {
    console.log(
      `[YouTube] Using user-supplied cookies (${cookieHeader.length} chars)`,
    );
  }
  ...
  for (const client of clients) {
    ...
    if (result.status === "OK") {
      ...
      console.log(
        `[YouTube] ${client.name} failed: ${result.status} - ${result.reason || "?"}`,
      );
...
function extractStreamsFromPlayerData(
  ...
  const videoDetails = (
    data as { videoDetails?: { title?: string; lengthSeconds?: string } }
  ).videoDetails;
  ...
  if (adaptive.length > 0) {
    console.log(
      `[YouTube] Formats: ${adaptive.length} total, ${withUrl} with direct URL, ${withCipher} with signatureCipher`,
    );
  }
...
async function fetchFromPiped(
  ...
  for (const source of sources) {
    try {
      ...
      console.log(
        `[Piped] ${source.name} error: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
  return null;
...
async function scanChunkForCandidates(
  ...
  for (let i = 1; i <= n; i++) {
    properties[`c${i}_title`] = {
      type: "string",
      description: `5-8 word title in ${targetLang} that maximizes curiosity`,
    };
    properties[`c${i}_hookLine`] = {
      type: "string",
      description: "Exact opening sentence copied verbatim from transcript",
    };
    properties[`c${i}_startSeconds`] = {
      type: "integer",
      description: "Start time in total seconds where the hook begins",
    };
    properties[`c${i}_endSeconds`] = {
      type: "integer",
      description: `End time in total seconds, ${minDuration}-${maxDuration}s after start`,
    };
    properties[`c${i}_viralScore`] = {
      type: "integer",
      description: "Honest viral potential 1-10. Most=5-7, exceptional=9-10",
    };
    properties[`c${i}_viralReasoning`] = {
      type: "string",
      description: `1-2 sentences in ${targetLang} explaining why this would perform well`,
    };
    properties[`c${i}_topicTags`] = {
      type: "string",
      description: "3-5 keyword tags, comma-separated",
    };
    required.push(
      `c${i}_title`,
      `c${i}_startSeconds`,
...
# LANGUAGE: titles, reasoning in ${targetLang}. hookLine verbatim from transcript.`,
  ...
  for (let i = 1; i <= n; i++) {
    ...
    const viralScore = Math.min(
      10,
      Math.max(1, Number(data[`c${i}_viralScore`] || 0)),
    );
...
> {
  const candidateSummary = candidates
    .map(
      (c, idx) =>
        `[${idx + 1}] "${c.title}" | ${c.startTime}s-${c.endTime}s | Score: ${c.viralScore}/10 | Hook: "${c.hookLine.substring(0, 100)}" | Why: ${c.viralReasoning.substring(0, 120)} | Tags: ${c.topicTags.join(", ")}`,
    )
    .join("\n");
  ...
  for (let i = 1; i <= numShorts; i++) {
    properties[`pick${i}_candidateNumber`] = {
      type: "integer",
      description: `Candidate number [N] for clip ${i}`,
    };
    properties[`pick${i}_title`] = {
      type: "string",
      description: `Polished 5-8 word title in ${targetLang}`,
    };
    properties[`pick${i}_caption`] = {
      type: "string",
      description: `1-line social media post in ${targetLang} with 2-3 hashtags`,
    };
    properties[`pick${i}_finalScore`] = {
      type: "integer",
      description: "Final viral score 1-10",
    };
    required.push(
      `pick${i}_candidateNumber`,
      `pick${i}_title`,
      `pick${i}_finalScore`,
    );
  }
...
${candidateSummary}`,
  ...
  if (result.error || !result.result) {
    ...
    return sorted.slice(0, numShorts).map(c => ({
      ...
      hashtags: c.topicTags.map(t => `#${t}`),
      startTime: c.startTime,
      endTime: c.endTime,
      transcriptExcerpt: c.hookLine,
      viralScore: c.viralScore,
      reason: c.viralReasoning,
      hookLine: c.hookLine,
      caption: c.caption,
  ...
  for (let i = 1; i <= numShorts; i++) {
    ...
    const finalScore = Math.min(
      10,
      Math.max(1, Number(data[`pick${i}_finalScore`] || original.viralScore)),
    );
    ...
    finalClips.push({
      ...
      hashtags:
        hashtagMatches.length > 0
          ? hashtagMatches
          : original.topicTags.map(t => `#${t}`),
      startTime: original.startTime,
      endTime: original.endTime,
      transcriptExcerpt: original.hookLine,
      viralScore: finalScore,
      reason: original.viralReasoning,
      hookLine: original.hookLine,
      caption: captionStr,
...
function correctTimestamps(
  ...
  return clips.map(clip => {
    ...
    for (const variant of [
      clip.hookLine,
      clip.hookLine.substring(0, 80),
      clip.hookLine.substring(0, 50),
      clip.hookLine.substring(0, 30),
    ]) {
    ...
    for (let i = timestampIndex.length - 1; i >= 0; i--) {
      if (timestampIndex[i].charPos <= hookPos) {
        nearestTs = timestampIndex[i];
        break;
      }
    }
    ...
    return {
      ...clip,
      startTime: nearestTs.seconds,
      endTime: nearestTs.seconds + dur,
    };
  });
}
...
function deduplicateCandidates(candidates: CandidateClip[]): CandidateClip[] {
  ...
  for (const c of sorted) {
    const existingIdx = result.findIndex(
      existing => Math.abs(existing.startTime - c.startTime) < 15,
    );
    if (existingIdx >= 0) {
      if (c.viralScore > result[existingIdx].viralScore)
...
> {
  ...
  const correctedCandidates = correctTimestamps(
    transcript,
    allCandidates.map(c => ({
      title: c.title,
      description: c.caption,
  ...
  const finalClips = await selectBestClips(
    deduped,
    videoTitle,
    videoAuthor,
    targetLang,
...
export const processJob = action({
  ...
  handler: async (ctx, { jobId }) => {
    ...
    try {
      ...
      if (!videoId)
        throw new Error("URL invalidă. Folosește un link YouTube valid.");
      ...
      console.log(
        `[processJob] Fetching video data via Piped for ${videoId}...`,
      );
      const piped = await fetchFromPiped(videoId);

      // Fallback to oEmbed for title if Piped fails
      const videoTitle =
        piped?.title || (await fetchVideoInfo(job.videoUrl, videoId)).title;
        ...
        try {
          ...
          if (subResp.ok) {
            ...
            if (transcriptResult.segments.length > 0) {
              console.log(
                `[processJob] Piped subtitles OK: ${transcriptResult.segments.length} segments`,
              );
            } else {
          ...
          console.log(
            `[processJob] Piped subtitles failed: ${err instanceof Error ? err.message : String(err)}`,
          );
        }
      }
      ...
      if (!transcriptResult) {
        try {
          ...
          if (transcriptResult) {
            console.log(
              `[processJob] YouTube captions OK: ${transcriptResult.segments.length} segments`,
            );
          }
        } catch (err) {
          console.log(
            `[processJob] YouTube captions failed: ${err instanceof Error ? err.message : String(err)}`,
          );
        }
      }
      ...
      if (!transcriptResult && openaiKey) {
        const audioUrl =
          piped?.audioUrl ||
          (await getCobaltDownloadUrl(job.videoUrl, "audio", cookieHeader));
        if (audioUrl) {
          try {
            console.log(
              "[processJob] Transcribing with Whisper (audio download)...",
            );
      ...
      if (clips.length === 0) {
        throw new Error(
          "Nu am putut identifica momente potrivite pentru Shorts.",
        );
      }
      ...
      if (!videoDownloadUrl) {
        try {
          ...
          console.log(
            `[processJob] InnerTube fallback failed: ${err instanceof Error ? err.message : String(err)}`,
          );
        }
      }
      ...
      await ctx.runMutation(internal.processing.updateJobStatus, {
        ...
        videoDownloadExpiry: videoDownloadUrl
          ? Date.now() + 3600000
          : undefined, // 1 hour
      });
    } catch (error) {
...
export const analyzeTranscript = internalAction({
  ...
  handler: async (ctx, { jobId, transcript }) => {
    const jobData = await ctx.runQuery(internal.processing.getJobInternal, {
      jobId,
    });
    ...
    try {
      ...
      if (clips.length === 0) {
        await ctx.runMutation(internal.processing.updateJobStatus, {
          jobId,
          status: "failed",
          error: "Nu am putut identifica momente potrivite.",
        });
